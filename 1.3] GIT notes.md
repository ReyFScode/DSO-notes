...
### Working with already created branches
To pull/work with/set up a sync with a non-main branch you must checkout, set the upstream origin to the branch (links to it), and then pull
```bash
git checkout nonMainBranch
git branch -u origin/nonMainBranch
git pull
```
...
```bash
git pull (optionally use http ref here)
```
